...
public class MoviesHandler extends BaseHttpHandler {
        ...
        if (path.equals("/movies")) {
            if (query != null) {
                try {
                    ...
                    if (moviesStore.getMoviesByYears(year).isEmpty()) {
                        sendJson(ex, 404, new ErrorResponse("Not Found", "Фильмы не найдены"));
                    } else {
                        HashMap<Integer, Movie> filtered = moviesStore.getMoviesByYears(year);
                        List<Movie> filteredList = new ArrayList<>(filtered.values());
                        sendJson(ex, 200, filteredList);
                    }
                } catch (NumberFormatException e) {
                    sendJson(ex, 400, new ErrorResponse("Bad Request", "Некорректный параметр запроса — 'year'"));
                }
            } else {
                if (moviesStore.getMovies().isEmpty()) {
                    ...
                    sendJson(ex, 200, moviesList);
                }
            }
        } else if (path.startsWith("/movies/")) {
            if (query != null) {
                sendJson(ex, 400, new ErrorResponse("Bad Request",
            ...
            try {
                ...
                if (moviesStore.getMovies().get(id) == null) {
                    ...
                    sendJson(ex, 200, moviesStore.getMovies().get(id));
                }
            } catch (NumberFormatException e) {
                sendJson(ex, 400, new ErrorResponse("Bad Request", "Некорректный ID"));
            }
        }
    }
    ...
    public void handlePostMethod(HttpExchange ex) throws IOException {
        if (!"/movies".equals(ex.getRequestURI().getPath())) {
            sendJson(ex, 404, new ErrorResponse("Not Found", "Объект не найден"));
            return;
        }
        ...
        try (InputStream inputStream = ex.getRequestBody()) {
            ...
            sendJson(ex, 422, new ErrorResponse("Unprocessable Entity", "Ошибка валидации \n" + e.getMessage()));
        }
    }
